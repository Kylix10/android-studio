...
public class DashboardFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        routePlanningButton.setOnClickListener(v -> {
            ...
            if (startPoint == null) {
                Toast.makeText(requireContext(), "请先设置起点", Toast.LENGTH_SHORT).show();
                showStartPointDialog();
            } else if (!flag1) {
                // 起点已设置，但还没设置途经点
                Toast.makeText(requireContext(), "请设置途经点（可选）", Toast.LENGTH_SHORT).show();
                showMidPointDialog();
            } else if (flag2 && endPoint == null) {
                // 选择了有终点模式，但终点未设置
                Toast.makeText(requireContext(), "请设置终点", Toast.LENGTH_SHORT).show();
                showEndPointDialog();
            } else {
                // 可以执行路线规划
    ...
    private void showRoutePlanningResultDialog() {
        ...
        // 检查是否有足够的点进行路径规划
        int totalPoints = 1 + midPoints.size() + (endPoint != null ? 1 : 0);
        if (totalPoints < 2) {
            // 只有起点，无法规划路径
            Toast.makeText(requireContext(), "至少需要两个点才能规划路径", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (result == null || result.path == null || result.path.size() < 2) {
    ...
    public PathResult optimizePathWithoutEnd(LatLng start, List<LatLng> waypoints,
        ...
        while (!pq.isEmpty()) {
            ...
            for (int i = 0; i < n; i++) {
                if (!visited[i]) {
                    pq.offer(new double[]{v, i, dist[v][i]});
                }
            }
        }
